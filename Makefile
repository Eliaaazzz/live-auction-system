...
WEB_SMOKE_USE_PRESET_AUCTION ?= 0
...
deploy-perf-rehearsal: ## #112: deploy + preflight + server-side SLO gate + optional client observed metrics pack
	...
	BASE_URL="$(BASE_URL)" \
	BASE_WS_URL="$(DEPLOY_REHEARSAL_BASE_WS_URL)" \
	AID="$(DEPLOY_REHEARSAL_AID)" \
	WS_PRECHECK_AUCTION="$(DEPLOY_REHEARSAL_AID)" \
	WS_PRECHECK_SCHEMA="$(DEPLOY_REHEARSAL_WS_SCHEMA)" \
