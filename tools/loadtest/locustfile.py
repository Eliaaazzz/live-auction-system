...
def _runner_stopping(env):
    # Once Locust starts teardown, recv errors usually come from the harness
    # closing blocked sockets, not from the server. Do not count those shutdown
    # artifacts as gateway failures in the final summary.
    ...
    def on_start(self):
        ...
        try:
            self.ws = websocket.create_connection(
                ...
                timeout=15,
                # Match the wsload harness for non-browser clients. The gateway
                # allows an empty Origin, but websocket-client otherwise sends
                # Origin: http://<host>, which breaks Docker-network targets such
                # as ws://lumen:8080 when FRONTEND_ORIGIN points elsewhere.
    ...
    def bid(self):
        ...
        while time.time() < deadline:
            ...
            if t == "BID_REJECTED":
                ...
                # rejects such as ERR_BAD_INPUT or ERR_AUCTION_PAUSED.
