...
- **`AUCTION_REVEALED`** is chained like any other event — the revealed sorted bid list (`[{userId, displayName, amountCents}]`) sits in `payload_json` and contributes to `event_hash`. For `VICKREY` the chain shows the winner alongside the second-highest top-level `amountCents` (the price paid); a verifier sees the full ordering, which is the audit anchor for "winner pays runner-up".
...
- **`ALL_PAY` compliance framing (mandatory on every surface).** UI labels (admin + room), the evidence card (the `settlement: "VIRTUAL_COINS_ONLY"` row), and any demo / presentation copy MUST mark an `ALL_PAY` auction as **「虚拟币 · 非真实支付 · 非赌博」** (virtual coins · non-payment · non-gambling). The Dollar-Auction mechanic (winner pays + runner-up forfeits) is a known game-theory pattern that reads as gambling without explicit framing; spec evaluators have to be able to tell at-a-glance that this is a virtual-coin demo, not a payment product. The `settlement` marker on the evidence card is the audit anchor for the framing — it is a **hard product gate**, not a stylistic preference (per @Eliaaazzz on #119).
