...
- **Typed close codes (T5-followup)**: WS spec reserves `4000-4999` for application use (RFC 6455 §7.4.2). `4000 BACKPRESSURE_DROP` is emitted on the critical-lane overflow path so a client can distinguish a server-initiated backpressure-drop from a raw network failure (back off + reconnect with state catchup, not a tight reconnect loop). Other server-side close paths use the implicit close (`code=1006` at the wire when the socket just closes).
