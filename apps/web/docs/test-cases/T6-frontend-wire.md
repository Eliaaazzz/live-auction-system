...
| TC-T6-116 | 单连接高频 BID_PLACE 被限流为 `ERR_RATE_LIMITED`：同连接 0.1s 内两次出价，后者 reject | `smoke:all`（含 `smoke:ratelimit`） + `npm run smoke:ratelimit`（后端 8080 起；`VERIFY_AID` 或 `AUCTION_ID` 指向真拍品） | P2 |
