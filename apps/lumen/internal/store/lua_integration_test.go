...
func TestSnapshotIncludesFrozenRules(t *testing.T) {
	...
	if snap.Rules.AuctionMode != model.AuctionModeSecondPrice {
		t.Fatalf("auctionMode=%s want %s", snap.Rules.AuctionMode, model.AuctionModeSecondPrice)
	}
}
...
func TestPlaceBidSecondPriceCapHitPaysRunnerUp(t *testing.T) {
	s := newTestStore(t)
	ctx := context.Background()
	r := defaultRules()
	r.CapPriceCents = 50000
	...
	if code, _, _, err := s.PlaceBid(ctx, aid, "u3", "cb3", "50000", "U3"); err != nil || code != model.CodeOKSold {
	...
	if len(events) != 4 ||
		events[3].Type != model.TypeAuctionSold ||
		events[3].ID != "4-0" ||
		events[3].Seq != 4 {
		t.Fatalf("stream=%+v", events)
	}
	...
	if sold.AmountCents != "12000" {
		t.Fatalf("auction sold amount=%s want 12000 (runner-up)", sold.AmountCents)
	}
}

func TestPlaceBidSecondPriceCapHitUsesReserveWhenRunnerUpBelowReserve(t *testing.T) {
	s := newTestStore(t)
	ctx := context.Background()
	r := defaultRules()
	r.CapPriceCents = 50000
	r.AuctionMode = model.AuctionModeSecondPrice
	aid := liveAuction(t, s, r, 60_000)
	if err := s.rdb.HSet(ctx, stateKey(aid), "reserveCents", "15000").Err(); err != nil {
		t.Fatal(err)
	}
	...
	if sold.AmountCents != "15000" {
		t.Fatalf("auction sold amount=%s want 15000 (runner-up below reserve)", sold.AmountCents)
	}
}

func TestPlaceBidSecondPriceCapHitFallsBackToReserveWhenNoRunnerUp(t *testing.T) {
	s := newTestStore(t)
	ctx := context.Background()
	r := defaultRules()
	r.CapPriceCents = 50000
	...
	if sold.AmountCents != "10000" {
		t.Fatalf("auction sold amount=%s want 10000 (reserve)", sold.AmountCents)
	}
}
...
func TestCloseAuctionSecondPriceUsesReserveWhenRunnerUpBelowReserve(t *testing.T) {
	s := newTestStore(t)
	ctx := context.Background()
	r := defaultRules()
	r.CapPriceCents = 0
	...
	if len(events) != 3 || events[2].Type != model.TypeAuctionSold || events[2].ID != "3-0" {
		t.Fatalf("events=%+v", events)
	}
	var sold model.AuctionSoldData
	if err := json.Unmarshal([]byte(events[2].Payload), &sold); err != nil {
		t.Fatal(err)
	}
	if sold.WinnerID != "u2" {
...
func TestCloseAuctionSecondPriceFallsBackToReserveWhenNoRunnerUp(t *testing.T) {
	s := newTestStore(t)
	ctx := context.Background()
	r := defaultRules()
	r.CapPriceCents = 0
