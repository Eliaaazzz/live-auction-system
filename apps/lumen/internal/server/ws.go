...
func (s *Server) handleWS(w http.ResponseWriter, r *http.Request) {
	...
	c := &Conn{ws: ws, send: make(chan []byte, sendBufFrames), lossy: make(chan []byte, 16), done: make(chan struct{}), userID: userID, displayName: display, pingPeriod: connPingPeriod}
	go c.writePump()
	defer func() {
		s.hub.leave(c)
		c.close()
