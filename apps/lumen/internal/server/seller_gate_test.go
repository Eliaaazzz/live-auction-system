...
func postProduct(t *testing.T, target, token string) int {
	...
	req, _ := http.NewRequest(http.MethodPost, target+"/api/products",
		bytes.NewReader([]byte(`{"name":"测试拍品","imageUrl":"http://img.local/x.jpg","description":"d"}`)))
...
func TestSellerKeyGateOnCreation(t *testing.T) {
	...
	resp, buyer := postLogin(t, target, `{"nickname":"老王"}`)
	...
	if resp, _ := postLogin(t, target, `{"nickname":"老王","sellerKey":"WRONG"}`); resp.StatusCode != http.StatusForbidden {
		t.Fatalf("wrong sellerKey status=%d want 403", resp.StatusCode)
	}

	// Right key: seller_* namespace, role seller — creation passes.
	resp, seller := postLogin(t, target, `{"nickname":"大掌柜","sellerKey":"demo-seller-key"}`)
	...
	_, mallory := postLogin(t, target, `{"nickname":"大掌柜"}`)
...
func TestSellerGateOpenModeWithoutKey(t *testing.T) {
	...
	_, buyer := postLogin(t, target, `{"nickname":"种子脚本"}`)
	...
	resp, seller := postLogin(t, target, `{"nickname":"卖家","sellerKey":"anything"}`)
	if resp.StatusCode != http.StatusOK || !strings.HasPrefix(seller.UserID, "seller_") || seller.Role != "seller" {
