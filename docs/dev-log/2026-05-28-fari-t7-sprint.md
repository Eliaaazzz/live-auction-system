...
- Guardrail: length ≤ 80 runes (NOT bytes — Chinese-char-friendly), URL regex (`(?i)\b(https?://|www\.)\S+`), phone regex (CN mobile + `+86` intl), money regex (`[¥$€£]\s*\d`), 6 banned marketing claims ("absolute lowest price" / "only one in existence" / "tenfold refund if fake" / "guaranteed authentic" / "100% genuine" / "buyback at original price"). Returns `(reason string, bad bool)` so sidecar log captures *which* rule fired.
...
- **B1 — Currency regex too narrow vs §2.4 stated intent**. Spec says "prevent commentary from naming alternative prices," but the regex was symbol-only — doesn't catch a yuan-suffixed price such as "starts at 1000 yuan" or `worth $500 USD`. A prompt injection asking the model to mention a 50000-yuan price leaks straight through. Mine #74 already catches all three (symbol prefix plus currency-word suffix). Suggested widening + 2 test cases.
