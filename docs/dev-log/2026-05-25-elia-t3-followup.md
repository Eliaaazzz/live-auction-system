...
- **TC-T3-100 (P1)** — DRAFT cancel + concurrent freeze TOCTOU. `freeze` and the DRAFT branch of `cancel` now share an auction transition lock; `handleCancel` also checks Redis state before any MySQL-only DRAFT cancel, so the Redis-frozen/MySQL-still-DRAFT projection window falls through to `cancel_auction.lua` instead of clobbering MySQL(CANCELLED) over Redis(SCHEDULED). Tests `TestT3CancelDraftConditionalUpdateGuardsTOCTOU` and `TestT3CancelDraftPathHonorsRedisFrozenWindow` pin both the MySQL CAS primitive and the Redis-first freeze window.
