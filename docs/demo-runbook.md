...
| 7.6 | 远端演练边界 | 按 [deploy rehearsal 卡片](deploy-rehearsal-card.md) 执行，或直接用 `make deploy-perf-rehearsal BASE_URL=<https://domain> [DEPLOY_REHEARSAL_AID=<auction>] [PERF_GATE_CLIENT_SUMMARY=<k6_summary.json>]` |  | 服务器端 SLO 与 client-observed 证据边界分离，避免把 WAN 延迟误判为后端瓶颈 |
